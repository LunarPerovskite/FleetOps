"""Claude Code Connector for FleetOps

Connects Anthropic's Claude Code to FleetOps governance platform.
"""
...
class ClaudeCodeConnector:
    """Connector for Claude Code agent"""
    
    PROVIDER = "anthropic"
    DEFAULT_MODEL = "claude-sonnet-4-6"
    
    def __init__(self, api_key: str, fleetops_url: str, org_id: str, 
                 agent_name: str = "Claude Code", agent_level: str = "senior",
        ...
        self.agent_id = None
        self.ws = None
        self.capabilities = ["code", "review", "debug", "refactor", "test"]
        self.sub_agents: Dict[str, 'ClaudeCodeConnector'] = {}
    ...
    async def _listen(self):
        ...
        try:
            async for message in self.ws:
                ...
                elif event_type == "subagent_assigned":
                    await self._handle_subagent_assigned(data)
                elif event_type == "task_assigned":
                    await self._handle_task_assigned(data)
                elif event_type == "heartbeat_ack":
                    pass
        except websockets.exceptions.ConnectionClosed:
            print(f"Claude Code connector disconnected")
    ...
    async def _handle_subagent_assigned(self, data: dict):
        """Handle sub-agent assignment from parent"""
        sub_agent_id = data.get("sub_agent_id")
        task_id = data.get("task_id")
        print(f"Sub-agent {sub_agent_id} assigned to task {task_id}")
    ...
    async def create_sub_agent(self, name: str, level: str = "junior"):
        ...
        sub_connector = ClaudeCodeConnector(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Claude Code FleetOps Connector")
    ...
    parser.add_argument("--name", default="Claude Code", help="Agent name")
    parser.add_argument("--level", default="senior", choices=["junior", "senior", "lead"])
    parser.add_argument("--parent-agent", help="Parent agent ID (for sub-agents)")
    ...
    print(f"Claude Code connector running. Agent ID: {connector.agent_id}")
